...
// --- NewsAPI Auto Import --- //
function import_news_from_newsapi() {
    $apiKey = '495e79676c5c4b00b6c7ce16de548481'; // Ganti dengan API key dari https://newsapi.org
    $response = wp_remote_get("https://newsapi.org/v2/top-headlines?country=id&apiKey=$apiKey");
    ...
    foreach ($data->articles as $article) {
        ...
        $post_content = sanitize_textarea_field(($article->description ?? '') . "\n\nSumber: " . $article->url);
...
// Jadwalkan cron event agar otomatis jalan tiap jam
if (!wp_next_scheduled('import_news_event')) {
    wp_schedule_event(time(), 'hourly', 'import_news_event');
}
add_action('import_news_event', 'import_news_from_newsapi');
